...
exports.connectSNSHttp = functions.https.onRequest(async (req, res) => {
    ...
    try {
                    ...
                    const result = await new Promise((resolve, reject) => {
                        oauth.getOAuthRequestToken(async (error, oauthToken, oauthTokenSecret, results) => {
                            if (error) {
                                ...
                                console.error("[connectSNSHttp] Error details:", JSON.stringify(error));
                                reject(error);
                                return;
                            }
                            try {
                                ...
                                await db
                                    ...
                                    .set(Object.assign(Object.assign({}, oauthData), { stateId: stateId }));
                                ...
                                console.error("[connectSNSHttp] Firestore save error:", firestoreError);
        ...
        if (sns_type !== "x") {
            console.log(`[connectSNSHttp] Generated auth URL for ${sns_type}: ${authUrl}`);
        }
        res.json({
            success: true,
            authUrl: authUrl,
...
exports.connectSNS = functions.https.onCall(async (data, context) => {
    ...
    if (!sns_type ||
        !["x", "instagram", "threads", "tiktok"].includes(sns_type)) {
        throw new functions.https.HttpsError("invalid-argument", "Invalid sns_type");
    }
    try {
        ...
        switch (sns_type) {
            case "x": {
                try {
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        oauth.getOAuthRequestToken(async (error, oauthToken, oauthTokenSecret, results) => {
                            if (error) {
                                ...
                                console.error("[connectSNS] Error details:", JSON.stringify(error));
                                reject(new functions.https.HttpsError("internal", `Twitter OAuth failed: ${JSON.stringify(error)}`));
                                return;
                            }
                            ...
                            try {
                                ...
                                await db
                                    ...
                                    .set(Object.assign(Object.assign({}, oauthData), { stateId: stateId }));
                                authUrl = `https://api.twitter.com/oauth/authenticate?oauth_token=${oauthToken}&state=${stateId}`;
                                console.log(`[connectSNS] Generated Twitter auth URL: ${authUrl}`);
                                resolve({
                                    success: true,
                                    authUrl: authUrl,
                                    sns_type: sns_type,
                                ...
                                console.error("[connectSNS] Firestore save error:", firestoreError);
                                reject(new functions.https.HttpsError("internal", "Failed to save OAuth state"));
                            }
                        });
                    });
                }
                    ...
                    throw new functions.https.HttpsError("internal", `OAuth setup failed: ${err}`);
                }
            }
            case "instagram":
                authUrl = `https://api.instagram.com/oauth/authorize?client_id=${process.env.INSTAGRAM_CLIENT_ID}&redirect_uri=${process.env.INSTAGRAM_CALLBACK_URL}&scope=user_profile,user_media&response_type=code`;
        ...
        if (sns_type !== "x") {
            console.log(`[connectSNS] Generated auth URL for ${sns_type}: ${authUrl}`);
        }
        return {
            success: true,
            authUrl: authUrl,
        ...
        switch (snsType) {
            case "x": {
                ...
                if (!oauth_token || !oauth_verifier) {
                    throw new Error(`Missing required OAuth parameters: oauth_token=${oauth_token ? "present" : "missing"}, oauth_verifier=${oauth_verifier ? "present" : "missing"}, state=${state ? "present" : "missing"}`);
                }
                ...
                if (state) {
                    ...
                    if (tokenDoc.exists) {
                        ...
                        const originalStateId = authData === null || authData === void 0 ? void 0 : authData.stateId;
                        if (originalStateId) {
                ...
                // アクセストークンを取得
                return new Promise((resolve, reject) => {
                    oauth.getOAuthAccessToken(oauth_token, authData.oauthTokenSecret, oauth_verifier, async (error, accessToken, accessTokenSecret, results) => {
                        try {
                            if (error) {
                                console.error("[handleSNSCallback] Twitter access token error:", error);
                                throw error;
                            }
                            // ユーザー情報を取得
                            oauth.get("https://api.twitter.com/1.1/account/verify_credentials.json", accessToken, accessTokenSecret, async (userError, userData) => {
                                try {
                                    if (userError) {
                                        console.error("[handleSNSCallback] Twitter user info error:", userError);
                                        throw userError;
                                    }
                        ...
                        <body>
                          <h2 class="${isExistingAccount ? "info" : "success"}">✅ ${statusMessage}</h2>
                      ...
                      `);
                                    ...
                                    console.error("[handleSNSCallback] Error saving account:", err);
                                    res
                                        .status(500)
                            ...
                            console.error("[handleSNSCallback] Access token processing error:", err);
            ...
            case "tiktok": {
                ...
                res.redirect(`${process.env.APP_URL || "http://localhost:4200"}/dashboard?sns_connected=${snsType}&status=not_implemented`);
                break;
            }
            default:
...
const authenticateUser = async (context) => {
    ...
    const authHeader = ((_b = (_a = context.rawRequest) === null || _a === void 0 ? void 0 : _a.headers) === null || _b === void 0 ? void 0 : _b.authorization) ||
        ((_d = (_c = context.rawRequest) === null || _c === void 0 ? void 0 : _c.headers) === null || _d === void 0 ? void 0 : _d.Authorization);
    if (authHeader && authHeader.startsWith("Bearer ")) {
        const idToken = authHeader.substring(7);
        try {
            const decodedToken = await admin.auth().verifyIdToken(idToken);
...
exports.getConnectedAccountsHttp = functions.https.onRequest(async (req, res) => {
    // CORS設定
    res.set("Access-Control-Allow-Origin", "*");
    res.set("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
    ...
    try {
        ...
        const authHeader = req.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            console.error("[getConnectedAccountsHttp] No valid authorization header found");
            res.status(401).json({
                error: "No valid authorization header found",
                received_header: authHeader,
            });
        ...
        const accounts = accountsSnapshot.docs.map((doc) => {
            ...
            return Object.assign(Object.assign({ id: doc.id }, data), { connected_at: ((_c = (_b = (_a = data.connected_at) === null || _a === void 0 ? void 0 : _a.toDate) === null || _b === void 0 ? void 0 : _b.call(_a)) === null || _c === void 0 ? void 0 : _c.toISOString()) || data.connected_at });
        });
        res.json({
            success: true,
            accounts: accounts,
...
exports.getConnectedAccounts = functions.https.onCall(async (data, context) => {
    try {
        ...
        throw new functions.https.HttpsError("internal", "Failed to fetch connected accounts");
    }
});
